...
/*
 * ❌ ОШИБКА: datatime_create(0, 0, 0, 0, 0) не проходит проверку check_time_format —
 *    month=0 и day=0 невалидны (m < 1 истинно), функция всегда возвращала NULL.
 *    Всё что написано ниже в main.c с empty_dt работало с NULL-указателем.
 *
 ...
 *    Поля заполняются нулями — это легальная "пустая" структура без смысловой даты.
 */
datatime* create_empty_datatime() {
    datatime *dt = malloc(sizeof *dt);
...
void datatimes_switch(datatime *dt1, datatime *dt2) {
    ...
    datatime temp;
    /*
     * ✅ В lab_1 поля — простые int, поэтому copy_datatime корректен.
     *    В lab_2 это же место содержит UB (см. lab_2/datatime.c).
     *    Здесь оставляем copy_datatime, но показываем эквивалент через memcpy:
